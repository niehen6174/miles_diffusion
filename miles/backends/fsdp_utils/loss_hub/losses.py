...
def flow_grpo_loss_formula(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        if len(ctx.models) > 1:
            log_stats[f"log_prob_mean_abs_diff_{prepared.component_name}"].append(log_prob_mean_abs_diff)

        rollout_model_output = stack_train_pair_rollout_debug(batch, "rollout_step_model_output")
        if rollout_model_output is not None:
            bsz = new_pred.shape[0]
            diff = (new_pred.reshape(bsz, -1).float() - rollout_model_output.to(ctx.device).reshape(bsz, -1).float()).abs()
            ref_max = rollout_model_output.reshape(bsz, -1).float().abs().max() + 1e-30
            mean_abs_diff = diff.mean().detach()
            log_stats["model_output_max_abs_diff"].append(diff.max().detach())
            log_stats["model_output_mean_abs_diff"].append(mean_abs_diff)
            log_stats["model_output_rel_max"].append((diff.max() / ref_max).detach())
            if len(ctx.models) > 1:
                log_stats[f"model_output_mean_abs_diff_{prepared.component_name}"].append(mean_abs_diff)
