...
def nft_loss_formula(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        per_pair_mean = per_pair.mean().detach()
        unirl_loss = per_pair_mean * float(max(num_timesteps, 1))
        log_stats["loss"].append(unirl_loss)
        log_stats["nft_loss"].append(unirl_loss)
        log_stats["nft_loss_per_pair"].append(per_pair_mean)
        log_stats["nft_r_mean"].append(r.mean().detach())
        log_stats["nft_pos_loss"].append(pos_loss.mean().detach())
        log_stats["nft_neg_loss"].append(neg_loss.mean().detach())
        log_stats["nft_adv_mean"].append(prepared.advantage.mean().detach())
        log_stats["nft_t_mean"].append(t.mean().detach())
        log_stats["nft_num_timesteps"].append(torch.tensor(float(num_timesteps), device=ctx.device, dtype=torch.float32))
        log_stats["adv_abs_mean"].append(prepared.advantage.abs().mean().detach())
